				...
				<tbody>
					...
					?>
						<tr>
							...
							<td ><p class="m-0"><?php echo ($row['type'] == 1 )? "Adminstrador" : "Autor" ?></p></td>
							<td class="text-center">
                                <?php
                                    switch($row['status']){
                                        case '1':
                                            echo "<span class='badge badge-success bg-primary badge-pill'>Verificad@</span>";
                                            break;
                                        case '0':
                                            echo "<span class='badge badge-secondary badge-pill'>Not Verificad@</span>";
                                            break;
                                    }
                                ?>
                            </td>
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("¿Está segur@ de eliminar este usuario de forma permanente?","delete_user",[$(this).attr('data-id')])
		})
		$('.table td,.table th').addClass('py-1 px-2 align-middle')
		$('.table').dataTable();
		$('.verify_user').click(function(){
			_conf("Estás segur@ de verificar este usuario <b>"+$(this).attr('data-name')+"<b/>?","verify_user",[$(this).attr('data-id')])
		})
	})
