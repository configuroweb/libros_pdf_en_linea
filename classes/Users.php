...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Detalles de Usuario guardados exitosamente');
				$resp['status'] = 1;
			}else{
				$resp['status'] = 2;
			}
			...
			if($qry){
				$this->settings->set_flashdata('success','Detalles de Usuario actualizados exitosamente');
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			if(!in_array($type,$allowed)){
				$resp['msg'].=" Carga de imagen fallo por tipo de archivo";
			}else{
				$new_height = 200; 
				$new_width = 200; 
	...
	public function delete_users(){
		...
		if($qry){
			...
			$this->settings->set_flashdata('success','Detalles de Usuario eliminados exitosamente');
